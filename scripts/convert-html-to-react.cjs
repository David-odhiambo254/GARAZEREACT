...
files.forEach(file => {
    ...
    // Remove preloader/header and mobile menu blocks so shared header/footer can be used
    body = body.replace(/<!--\s*preloader\s*-->[\s\S]*?<!--\s*main-header end\s*-->/i, '');
    body = body.replace(/<!--\s*Mobile Menu[\s\S]*?<!--\s*End Mobile Menu\s*-->/i, '');
    // Remove footer block
    body = body.replace(/<!--\s*main-footer\s*-->[\s\S]*?<!--\s*main-footer end\s*-->/i, '');
